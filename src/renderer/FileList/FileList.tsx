...
export const FileList = memo(function FileList({ currentPath, parentNode, entries, onNavigate, onViewFile, active, resolver }: FileListProps) {
  ...
  // Refs for values needed at action-execution time
  ...
  // Keep topmostIndex in sync with activeIndex: only move when activeIndex goes off-screen.
  // topmostIndex never leads or lags — it stays put until the cursor leaves the visible window.
  useEffect(() => {
    setTopmostIndex((t) => {
  ...
  const handleItemsPerColumnChanged = useCallback((count: number) => {
    ...
    // Clamp topmostIndex so it stays within new bounds (prevents y-shift on resize)
